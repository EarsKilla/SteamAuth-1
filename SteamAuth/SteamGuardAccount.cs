	...
	public class SteamGuardAccount {
		...
		public bool DeactivateAuthenticator(int scheme = 2) {
			...
			try {
				...
				return removeResponse != null && removeResponse.Response != null && removeResponse.Response.Success;
			} catch (Exception) {
				return false;
			}
		}
		...
		public long GetConfirmationTradeOfferID(Confirmation conf) {
			if (conf.ConfType != Confirmation.EConfirmationType.Trade) {
		...
		public bool AcceptMultipleConfirmations(Confirmation[] confs) => SendMultiConfirmationAjax(confs, "allow");

		public bool DenyMultipleConfirmations(Confirmation[] confs) => SendMultiConfirmationAjax(confs, "cancel");

		public bool AcceptConfirmation(Confirmation conf) => SendConfirmationAjax(conf, "allow");

		public bool DenyConfirmation(Confirmation conf) => SendConfirmationAjax(conf, "cancel");
		...
		private ConfirmationDetailsResponse GetConfirmationDetails(Confirmation conf) {
			...
			ConfirmationDetailsResponse confResponse = JsonConvert.DeserializeObject<ConfirmationDetailsResponse>(response);
			return confResponse;
		}
		*/
