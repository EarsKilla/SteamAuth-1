	...
	public class UserLogin {
		...
		public ELoginResult DoLogin() {
			...
			if (response == null || response.Contains("<BODY>\nAn error occurred while processing your request.")) {
				return ELoginResult.GeneralFailure;
			}

			RSAResponse rsaResponse = JsonConvert.DeserializeObject<RSAResponse>(response);

			if (!rsaResponse.Success) {
				return ELoginResult.BadRSA;
			}
			...
			if (loginResponse.TwoFactorNeeded && !loginResponse.Success) {
				...
				SessionData session = new SessionData {
					SteamID = ulong.Parse(readableCookies["SteamLoginSecure"].Value.Split('%')[0]),
			...
			if (!loginResponse.LoginComplete) {
				...
				CookieCollection readableCookies = cookies.GetCookies(new Uri("https://steamcommunity.com"));
				LoginResponse.OAuth oAuthData = loginResponse.OAuthData;
